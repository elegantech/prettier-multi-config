...
if (!found || found !== inputPrettierConfigPath) {
  ...
  console.error(
    chalk.yellowBright("Did you forget to run `npm run build:internal`?"),
  );
...
try {
  ...
  console.error(
    chalk.redBright("Unable to convert the config file to JSON format."),
  );
...
console.log(
  chalk.greenBright('  Config file "%s" created successfully!'),
  relativeJsonPrettierConfigPath,
);
...
const relativePrettierConfigPath = path.relative(
  ROOT_DIR,
  outputPrettierConfigPath,
);
